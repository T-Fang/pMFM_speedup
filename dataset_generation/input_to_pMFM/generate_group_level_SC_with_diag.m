function group_level_SC_with_diag = generate_group_level_SC_with_diag(subject_list)
    ...
    group_level_SC_with_diag = zeros(68);
    ...
    for i = 1:68
        ...
        for j = 1:68
            ...
            group_level_SC_with_diag(i, j) = mean(nonzeros(SC_entry_all_log));
        end

    end

    group_level_SC_with_diag(isnan(group_level_SC_with_diag)) = 0;
    % group_level_SC_with_diag(logical(eye(68))) = 0; % comment this out if we want diagonal SC entries

    roi_list = (1:68) .* roi_list';
    roi_list(roi_list == 0) = [];
    roi_list = setdiff(1:68, roi_list); %roi remove list

    group_level_SC_with_diag(roi_list, :) = [];
    group_level_SC_with_diag(:, roi_list) = [];
end
